...
To handle large datasets, I would opt to swap the framework from Flask to something that doesn't utilise Python as Python can take quite a bit longer than other languages. I would likely swap to either Go or Rust as they would make SQL queries and handling a database more efficient than Flask
...
- Creating an actual database. At present data is directly read from a single excel file which makes scaling this data somewhat cumbersome and difficult to do. If I had more time and the resources to do it, I would've designed a database schema for this task and implemented the data into it
